...
class DecisionTree(BaseEstimator, RegressorMixin):  # Inherits from sklearn to use their Cross Validation methods
    ...
    def find_best_splitting_point(self, X, y):
        ...
        split_index = None
        split_value = None
        loss = n * DecisionTree.compute_loss(y)  # loss = SSE = sum (y_i - y.mean)^2
        ...
        for var_index in range(p):
            ...
            for i in range(self._min_leaf - 1, n - self._min_leaf):
                ...
                lhs_count, lhs_loss = i+1, DecisionTree.compute_loss(sorted_y[:(i + 1)])
                rhs_count, rhs_loss = n-i-1, DecisionTree.compute_loss(sorted_y[(i + 1):])
