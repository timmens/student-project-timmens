...
class DecisionTree:
    ...
    def get_level_in_list(tree, level):
        ...
        if level == 0:
            ...
            if tree.left_child is not None:
                level_list.extend(DecisionTree.get_level_in_list(tree.left_child, level - 1))
                level_list.extend(DecisionTree.get_level_in_list(tree.right_child, level - 1))
        return level_list
    ...
    def get_pruned_tree_and_alpha_sequence(fitted_tree, thresh):
        ...
        assert isinstance(fitted_tree, DecisionTree), 'This method only works on Decision Trees'
        ...
        alphas = [0]
        subtrees = [DecisionTree.get_first_subtree(fitted_tree, thresh)]  # get_first_subtree() does deepcopy
    ...
    def get_optimal_subtree_via_k_fold_cv(X_learn, y_learn, k=5, thresh=0, fitted_tree=None):
        ...
        tree_max_sequences = DecisionTree.get_pruned_tree_and_alpha_sequence(tree_max, thresh)
        ...
        for tree in tree_k_max_list:
            tree_k_subtree_dict.append(DecisionTree.get_pruned_tree_and_alpha_sequence(tree, thresh))
        ...
        for alpha in alphas:
            ...
            for k, cv_tree in enumerate(tree_k_max_list):
                cv_alpha_subtree = DecisionTree.get_subtree_corresponding_to_arbitrary_alpha(cv_tree, alpha, thresh)
                err_alpha += DecisionTree.validate(cv_alpha_subtree, test_X[k], test_y[k])
