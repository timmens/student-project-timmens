plot_AW <- function(smooth_param)

param <- as.list(match.call())  
smooth_param. <- param$smooth_param

tau <- function(x1,x2, smooth_param. ){
  #(1+(1+exp(-20*(x1-1/3)))^(-1))*(1+(1+exp(-20*(x2-1/3)))^(-1))
  (1+(1+exp(-smooth_param.*(x1-1/3)))^(-1))*(1+(1+exp(-smooth_param.*(x2-1/3)))^(-1))
}
